...
def log_validation(
    ...
    # run inference
    generator = (
        torch.Generator(device=accelerator.device).manual_seed(hyperparameters.seed)
        if hyperparameters.seed
        else None
    )
            ...
            tracker.log(
                {
                    phase_name: [
                        wandb.Image(
                            image, caption=f"{i}: {hyperparameters.validation_prompt}"
                        )
                        for i, image in enumerate(images)
                    ]
                }
...
class DreamBoothTrainer:
    def __init__(self, hyperparameters):
        ...
        for hyperparameter_name in self.default_hyperparameters:
            if hyperparameter_name not in self.hyperparameters:
                self.hyperparameters[hyperparameter_name] = (
                    self.default_hyperparameters[hyperparameter_name]
                )

        set_seed(self.hyperparameters.seed)
        self.logging_dir = Path(
            self.hyperparameters.output_dir, self.hyperparameters.logging_dir
        )
    ...
    def generate_class_images(self):
        ...
        if cur_class_images < self.hyperparameters.num_class_images:
            ...
            sample_dataset = PromptDataset(
                self.hyperparameters.class_prompt, num_new_images
            )
            sample_dataloader = torch.utils.data.DataLoader(
                sample_dataset, batch_size=self.hyperparameters.sample_batch_size
            )
    ...
    def prepare_dataset(self, tokenizer, text_encoder):
        ...
        pre_computed_encoder_hidden_states = compute_text_embeddings(
            self.hyperparameters.instance_prompt
        )
        ...
        if self.hyperparameters.class_prompt is not None:
            pre_computed_class_prompt_encoder_hidden_states = compute_text_embeddings(
                self.hyperparameters.class_prompt
            )
        else:
        ...
        train_dataset = DreamBoothDataset(
            ...
            class_data_root=(
                self.hyperparameters.class_data_dir
                if self.hyperparameters.with_prior_preservation
                else None
            ),
        ...
        train_dataloader = torch.utils.data.DataLoader(
            ...
            collate_fn=lambda examples: collate_fn(
                examples, self.hyperparameters.with_prior_preservation
            ),
        )
...
def train(hyperparameters):
    ...
    noise_scheduler = DDPMScheduler.from_pretrained(
        trainer.hyperparameters.pretrained_model_name_or_path, subfolder="scheduler"
    )
    ...
    progress_bar = tqdm(range(0, trainer.hyperparameters.max_train_steps), desc="Steps")
    ...
    for epoch in range(0, trainer.hyperparameters.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with trainer.accelerator.accumulate(unet):
                ...
                loss = (
                    instance_loss
                    + trainer.hyperparameters.prior_loss_weight * prior_loss
                )
